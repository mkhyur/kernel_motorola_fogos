...
static s64 mem_cgroup_version_read(struct cgroup_subsys_state *css,
		struct cftype *cft)
{
	return MOTO_SWAP_VERSION;
}

struct cftype mem_cgroup_swapd_legacy_files[] = {
	{
		.name = "moto_swap_version",
		.flags = CFTYPE_ONLY_ON_ROOT,
		.read_s64 = mem_cgroup_version_read,
	},
	{
...
static inline u64 calc_shrink_scale(pg_data_t *pgdat)
{
	...
	while ((memcg = fetch_next_memcg(memcg))) {
		...
		hybp(HYB_DEBUG, "CHECK MEMCG %s: can_reclaim %luKB nr_anon %luKB zram %luKB eswap %luKB total %luKB reclaimed %lu%%(%lu)\n",
				...
				(unsigned long)((nr_zram + nr_eswap) * 100 / (total + 1)),
...
static int swapd_run_locked(int nid)
{
	...
	atomic_set(&hyb_task->swapd_wait_flag, 0);
	hyb_task->swapd = kthread_create(swapd, pgdat, "mswapd:%d", nid);
